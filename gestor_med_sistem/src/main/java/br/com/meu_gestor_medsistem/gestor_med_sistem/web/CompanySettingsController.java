...
import br.com.meu_gestor_medsistem.gestor_med_sistem.domain.CompanySettings;
import br.com.meu_gestor_medsistem.gestor_med_sistem.repository.CompanySettingsRepository;
...
public class CompanySettingsController {
    ...
    public CompanySettingsController(CompanySettingsRepository repository) {
    ...
    public CompanySettings criar(@Valid @RequestBody Request body) {
        if (repository.existsById(body.companyId()))
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Configuração já existe");
        return repository.save(preencher(new CompanySettings(), body));
    }
    ...
    public void excluir(@PathVariable UUID companyId) {
        repository.delete(encontrar(companyId));
    }
